...
const AuthPanel = ({ activeRole, onSelectRole, onProviderLogin, onManualLogin, error }) => {
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {providers.map((provider) => (
          <motion.button
            ...
            className="rounded-3xl border border-white/10 bg-gradient-to-r from-amber-400/40 via-transparent to-white/10 px-4 py-4 text-sm font-medium text-white/85 shadow-inner transition hover:border-amber-300/60"
          >
            {provider.label}
          </motion.button>
        ))}
      ...
      <div className="space-y-4 rounded-3xl border border-white/10 bg-white/10 p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <input
            ...
            placeholder="email"
            className="w-full rounded-2xl border border-white/10 bg-black/60 px-4 py-3 text-sm uppercase tracking-[0.35em] text-white focus:border-amber-300/80 focus:outline-none"
          />
          <input
            type="password"
            value={password}
